...
wH = 1/(R2*C2)
wO1 = sqrt(wL*wH)
AV_HP = abs((R1*C1*j*wO1)/(1+R1*C1*j*wO1))
AV_HPdb = 20*log10(AV_HP)
AV_Amp = (1+R4/R3)
AV_Ampdb = 20*log10(AV_Amp)
AV_LP = abs(1/(1+R2*C2*j*wO1))
AV_LPdb = 20*log10(AV_LP)
AV = AV_HP*AV_Amp*AV_LP
AVdb1 = AV_HPdb + AV_Ampdb + AV_LPdb
...
AV_LPdb = 20*log10(AV_LP)
AV = AV_HP*AV_Amp*AV_LP
AVdb2 = AV_HPdb + AV_Ampdb + AV_LPdb
z_in = R1 + 1/(j*wO*C1)
z_out = R2/(1+j*wO*R2*C2)

cost = 13323.29204 + (R1+R2+R3+(R4-50000)*3)*0.001 + (C1+(C2*2)*2)*1000000
merit = 1/(cost*(abs(AV-100)+abs((wO1/(2*pi))-1000)+(1e-6)))

fid = fopen("theo_tab.tex","w")
fprintf(fid, "Gain (1 KHz) & %f \\\\ \\hline \n", AV)
fprintf(fid, "Gain (1 KHz)(dB) & %f dB \\\\ \\hline \n", AVdb2)
fprintf(fid, "Gain (calculated central frequency) (dB) & %f dB \\\\ \\hline \n", AVdb1)
